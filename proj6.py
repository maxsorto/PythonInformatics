...
file = open('output.txt', 'w') # Open file
...
def assign_results():
	...
	choldata.append([patient_id, ch, ch_cat, ldl_input, l_cat, hdl_input, h_cat, trig_input, t_cat])
...
def dictcounts(a_list, i):

	dict1 = {}							# Create empty dictionary...
	
	for item in a_list: 				# Loop through items in list...
		if item[i] not in dict1: 		# If item at index is not in dictionary add it, if it is add to its count...
			dict1[item[i]] = 1
		else:
			dict1[item[i]] += 1
...
def createdictcounts():

	a = dictcounts(gendata, 1)
	b = dictcounts(bmidata, 4)
	c = dictcounts(choldata, 2)
	d = dictcounts(choldata, 4)
	e = dictcounts(choldata, 6)
	f = dictcounts(choldata, 8)
	g = dictcounts(bpdata, 2)
	h = dictcounts(bpdata, 4)


	dict_list = [a, b, c, d, e, f, g, h]

	return dict_list

def findnumbersummary(a_list, i):
	...
	dict2 = {'Avg': av, 'Max': mx, 'Median': md, 'Min': mn, 'Std Dev': sd}

	return dict2


def createnumbersummaries():

	bm = findnumbersummary(bmidata, 1)
	he = findnumbersummary(bmidata, 2)
	we = findnumbersummary(bmidata, 3)
	tc = findnumbersummary(choldata, 1)
	ld = findnumbersummary(choldata, 3)
	hd = findnumbersummary(choldata, 5)
	tr = findnumbersummary(choldata, 7)
	sy = findnumbersummary(choldata, 1)
	di = findnumbersummary(choldata, 3)

	dict3 = {'BMI': bm, 'Height': he, 'Weight': we, 'Triglycerides': tr, 'Systolic BP': sy, 'Diastolic BP': di}
...
def printstatistics():
	...
	counts = createdictcounts()
	summaries = createnumbersummaries()

	print "Frequencies by Categories"
	print "\tGender"
	printdict(counts[0])
	print "\n\tBMI"
	printdict(counts[1])	
	...
	print "Numeric Summaries by Health Indicator"
	print "\tBMI"
	printdict(summaries['BMI'])
	print "\n\tHeight"
	printdict(summaries['Height'])	
	print "\n\tWeight"
	printdict(summaries['Weight'])
	print "\n\tTriglycerides"
	printdict(summaries['Triglycerides'])
	print "\n\tSystolic BP"
	printdict(summaries['Systolic BP'])
	print "\n\tDiastolic BP"
	printdict(summaries['Diastolic BP'])
